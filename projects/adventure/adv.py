...
world.printRooms()
player = Player("daniel", world.startingRoom)
traversalPath = []

# TODO: when visited an ajoined room which happens to be multi connected it will show visited for all sometimes, at which point i go in wrong directions.  how can i change this?
'''
def trav(player):
    global traversalPath
    info, s, choice, reverse_dirs = {}, Queue(), False, {'n':'s', 's':'n', 'e':'w', 'w':'e'}
    s.enqueue([player.currentRoom])
    while len(info) < len(roomGraph):
        i = s.dequeue()
        last = i[-1]
        if last not in info:
            info[last] = [[x, False] for x in last.getExits()]
            if choice:
                info[last][info[last].index([reverse_dirs[choice], False])] = [reverse_dirs[choice], True]
        new, old = ([x[0] for x in info[last] if x[1] == False]), ([x[0] for x in info[last] if x[1] == True])#new is nonvisited history
        if len(new) > 0:
            choice = random.choice(new)
            index = info[last].index([choice, False])
            info[last][index] = [choice, True]
        else:
            f = reverse_dirs[choice]
            if len(old) > 1:
                choices = set(old) ^ set(f)
                print(f'possible choices {choices}')
                choice = random.choice(list(choices))
                print(f'decided {choice}')
            else:
                print(f'trying {choice}')
                choice = random.choice(old)
        traversalPath.append(choice)
        player.travel(choice)
        i = i+[player.currentRoom]
        s.enqueue(i)'''

# ==================================================
# START OVER MORON, BREAK IT DOWN INTO SMALLER PARTS
# ==================================================

# go north until i cant
# from there go east until i can go north
# repeat until i add in w and s
# if room completely visited bfs to find earliest previous room with ?
# go there
...
def add_room(room): # making sure i have a dictionary with right info 
    graph[room] = {}
    for direction in player.currentRoom.getExits():
        graph[player.currentRoom.id][direction] = '?'

add_room(room)

def bfs(room): #from mondays work, few changes
    visited = set()
    q = Queue()
    q.enqueue([room])
    while q.size() > 0:
...
while len(graph) < len(roomGraph):
    ...
    for direction in graph[room]:
        if graph[room][direction] == '?': #exploring unvisited
            choice = direction
            if choice:
                traversalPath.append(choice)
                player.travel(choice)
                ...
                if newroom not in graph:
                    add_room(newroom)
            graph[room][choice] = newroom
            graph[newroom][rev_dir[choice]] = room
            room = newroom
            break
    path = bfs(room) #backtracking
    if path:
        for step in path:
            for direction in graph[room]:
                if graph[room][direction] == step:
